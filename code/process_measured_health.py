...
def master_function(measures, data_dirs, save_dir, first_day, last_day, miRNA, fluor_measures=False, auto_measures=False, size_measures=False,control_for=None):
	
	#finds average values for each measure on each day b/t first and last day
	all_means=[]
	for i in range(0, len(measures)):
		means, lifespans, worms=get_data(data_dirs, save_dir, measures[i], first_day, last_day)
...
def make_aggregate_plot(megalist,days, miRNA,measures):
	...
	mediumseagreen_patch=mpatches.Patch(color='mediumseagreen', label=terms[measures[0]])
	deepskyblue_patch=mpatches.Patch(color='deepskyblue', label=terms[measures[1]])
	gray_patch=mpatches.Patch(color='gray', label=terms[measures[2]])
	aqua_patch=mpatches.Patch(color='aqua', label=terms[measures[3]])
	mediumspringgreen_patch=mpatches.Patch(color='mediumspringgreen', label=terms[measures[4]])
	goldenrod_patch=mpatches.Patch(color='goldenrod', label=terms[measures[5]])
	navy_patch=mpatches.Patch(color='navy', label=terms[measures[6]])
	black_patch=mpatches.Patch(color='black', label=terms[measures[7]])
	magenta_patch=mpatches.Patch(color='magenta', label=terms[measures[8]])
	purple_patch=mpatches.Patch(color='purple', label=terms[measures[9]])
	#orange_patch=mpatches.Patch(color='orange', label=terms[measures[10]])

	many_patches=[mediumseagreen_patch,deepskyblue_patch,gray_patch,aqua_patch,mediumspringgreen_patch,goldenrod_patch,navy_patch,black_patch,magenta_patch,purple_patch]
	
	less_patches=[]
	for i in range(0, len(megalist)):
		less_patches.append(many_patches[i])
	plt.legend(handles=less_patches, loc='upper right')
	plt.title('Correlation of '+miRNA +' expression with lifespan vs. timepoint measured', y=1.05,fontdict={'size':14, 'family':'calibri'})
	ymin, ymax = plt.ylim()
...
def make_correlation_plots(correlation_list, days,measure, miRNA, save_dir):
	plt.style.use('seaborn-white')
	plt.plot(days,correlation_list, c='mediumseagreen')
	plt.title('Correlation of '+miRNA + ' expression ' + '('+ terms[measure]+')'+ ' with lifespan vs. timepoint', y=1.05, fontdict={'size':14,'family':'calibri'})
	plt.xlabel('Timepoint (dph)', fontdict={'size':12,'family':'calibri'})
	plt.ylabel('Coefficient of determination (r)',fontdict={'size':12,'family':'calibri'})
	axis=plt.gca()
	axis.spines['right'].set_visible(False)
	axis.spines['top'].set_visible(False)
	ymin, ymax = plt.ylim()
...
def organize(data_dirs):
	work_dir='/Volumes/scopearray/Kinser_Holly/work_dir' 
	human_dir='/Volumes/scopearray/Kinser_Holly/utilities'
	extra_dirs=[]
	experiment_dirs=[]
	annotations_dirs=[]
...
def get_data(data_dirs,save_dir, select_measure,first_day,last_day):
